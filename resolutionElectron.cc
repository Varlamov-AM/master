...
#include <math.h>
Double_t smearP(Double_t, Double_t, int);

// This function generates smeared electron 4-momentum from the true one

TLorentzVector resolutionElectron(TLorentzVector pTrue, int smear_type)
{
  // Get particle mass
  ...
  Double_t p3Smeared = smearP(p3True, pTrue.E(), smear_type);
  ...
  // if smear_type == 2 (PHOS-type smearing) calculate direction smearing
